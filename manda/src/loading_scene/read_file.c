...
/*   Updated: 2025/03/08 00:49:16 by dath             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	load_data_from_file(t_gfx *gfx, char *filename)
{
	...
	if (!result)
		return (0);
	result = validate_and_parse_map(&gfx->map);
	if (result)
	{
		gfx->player.pos
			= vec2(0.5f + gfx->map.start_pos.x, 0.5f + gfx->map.start_pos.y);
		gfx->player.dir = cardinal_to_angle(gfx->map.start_orientation);
	}
	return (result);
}

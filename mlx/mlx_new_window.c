...
#include <X11/Xatom.h>
void set_fullscreen(t_xvar *xvar, t_win_list *new_win)
{
    XMapWindow(xvar->display, new_win->window);

    Atom wm_state = XInternAtom(xvar->display, "_NET_WM_STATE", False);
    Atom wm_fullscreen = XInternAtom(xvar->display, "_NET_WM_STATE_FULLSCREEN", False);

    XChangeProperty(xvar->display, new_win->window, wm_state, XA_ATOM, 32, PropModeReplace,
...
void	*mlx_new_window(t_xvar *xvar,int size_x,int size_y,char *title, int fullscreen)
{
	t_win_list				*new_win;
